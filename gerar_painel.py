...
.chk.auto .ponto { width: 8px; height: 8px; border-radius: 50%; background: var(--ink-faint); flex: none; }
.chk.auto.on .ponto { background: var(--good); }
...
function montarConteudo(rca) {
  ...
  const checks = [
    ...
    // original) — só mostra se bateu ou não, calculado direto do dado.
    const bateuRecompra = rca.recompra_pct < DADOS.constantes.recompra_limite;
    const on = bateuRecompra ? "on" : "";
    return `<span class="chk auto ${on}"><span class="ponto"></span> Recompra ${fmtPct(rca.recompra_pct)} ${bateuRecompra ? "(bateu)" : "(não bateu)"}</span>`;
  })();
