...
.chk.auto input { accent-color: var(--good); }
...
function montarConteudo(rca) {
  ...
  const checks = [
    ...
    return `<label class="chk auto ${on}">
      <input type="checkbox" disabled ${bateuRecompra ? "checked" : ""}> Recompra ${fmtPct(rca.recompra_pct)} ${bateuRecompra ? "(bateu)" : "(não bateu)"}
    </label>`;
  })();
