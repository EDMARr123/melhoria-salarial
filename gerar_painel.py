...
.media-pedidos-linha {
  margin-top: 6px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: .02em;
  color: var(--accent);
}
    ...
    <div class="panel">
      <div class="busca-row" style="align-items:flex-start">
        ...
        <div class="campo">
          ...
          <div class="media-pedidos-linha">QTD NO MÊS: ${rca.total_pedidos}</div>
        </div>
      </div>
    </div>
...
document.getElementById("conteudo").addEventListener("input", (e) => {
  if (e.target.classList.contains("meta-posit-input")) {
    const cfg = lerConfig();
    cfg.metasCategoria[e.target.dataset.chave] = parseNum(e.target.value);
    salvarConfig(cfg);
    renderizarRca();
